...
def run_linear(
    ...
    linear_layer = Linear(d_in, d_out, **factory_kwargs)
...
def run_embedding(
    ...
    """
    embedding_layer = Embedding(vocab_size, d_model, **factory_kwargs)
    state_dict = {"weight" : weights}
...
def run_swiglu(
    ...
    swiglu_layer= SwiGLU(d_model, d_ff, **factory_kwargs)
...
def run_rope(
    ...
    """
    rope_encode = RoPE(theta, d_k, max_seq_len, device="cpu")
    return rope_encode(in_query_or_key, token_positions)
...
def run_transformer_block(
    ...
    """
    rope = RoPE(theta, d_model // num_heads, max_seq_len, **factory_kwargs)
    transformer_block_layer = TransformerBlock(d_model, num_heads, d_ff, max_seq_len, rope, **factory_kwargs)
    transformer_block_layer.load_state_dict(weights)
...
def run_transformer_lm(
    ...
    for k, v in weights.items():
        print(f"k_name {k}, shape {v.shape}")
    transformer_model = TransformerLM(d_model, num_heads, num_layers, d_ff, context_length, rope_theta, vocab_size, **factory_kwargs)
    transformer_model.load_state_dict(weights)
    return transformer_model(in_indices)
...
def run_rmsnorm(
    ...
    """
    factory_kwargs = {"device" : "cpu", "dtype" : torch.float32}
    rmsnorm_layer = RMSNorm(d_model, eps, **factory_kwargs)
    state_dict = {"weight" : weights}
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    vocab, merges = train_bpe(input_path, vocab_size, special_tokens, 4)
    return vocab, merges
